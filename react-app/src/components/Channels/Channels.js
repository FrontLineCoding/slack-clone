    ...
    return (
        <div className='channels'>
            <div className='show-channels'>
                ...
                {isOwned && <button onClick={(() => {setShowModal(true)})}><img src={add} className='channel-add'></img></button>}
            ...
            {channelsArr?.map(channel => {
                return (
                    <div className={`single-channel`}>
                        ...
                        {isOwned &&<div className='options hide'>
                            <img src={cancel} className={`${channel.id}`} onClick={(e) => {deleteChannel(channel.id)}}></img>
                            <img src={edit} className='channel-edit' onClick={((e) => handleChannelEditClick(channel.id))}></img>
                        </div>}
                    </div>
                )
            })}
